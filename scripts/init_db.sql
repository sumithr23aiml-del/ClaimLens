...
CREATE TABLE IF NOT EXISTS policies (
    ...
    product_id VARCHAR(30) NOT NULL REFERENCES policy_products(product_id),
...
CREATE TABLE IF NOT EXISTS coverages (
    ...
    coinsurance_pct NUMERIC(5,2) NOT NULL DEFAULT 0 CHECK (coinsurance_pct BETWEEN 0 AND 100),
...
CREATE TABLE IF NOT EXISTS claims (
    ...
    claimed_amount NUMERIC(14,2) CHECK (claimed_amount >= 0),
    description TEXT,
    status VARCHAR(24) NOT NULL DEFAULT 'registered'
        CHECK (status IN ('registered','documents_pending','extracted','assessed',
                          'decided','settled','rejected','withdrawn')),
    assigned_to VARCHAR(80)
);

CREATE INDEX IF NOT EXISTS idx_claims_status ON claims(status, reported_at DESC);
CREATE INDEX IF NOT EXISTS idx_claims_policy ON claims(policy_id, loss_date DESC);
...
CREATE TABLE IF NOT EXISTS policy_snapshots (
    ...
    inception_date DATE NOT NULL,
    expiry_date DATE NOT NULL,
    terms JSONB NOT NULL, -- full coverage set as at the loss date
...
CREATE TABLE IF NOT EXISTS claim_documents (
    ...
    ocr_status VARCHAR(20) NOT NULL DEFAULT 'pending'
        CHECK (ocr_status IN ('pending','done','failed','not_required')),
    uploaded_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (claim_id, checksum_sha256) -- the same file cannot be submitted twice
);

CREATE INDEX IF NOT EXISTS idx_docs_claim ON claim_documents(claim_id, document_type);

CREATE TABLE IF NOT EXISTS extracted_fields (
    field_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    claim_id UUID NOT NULL REFERENCES claims(claim_id) ON DELETE CASCADE,
    document_id UUID NOT NULL REFERENCES claim_documents(document_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_fields_claim ON extracted_fields(claim_id, field_name);

CREATE TABLE IF NOT EXISTS claim_line_items (
    line_id BIGSERIAL PRIMARY KEY,
    claim_id UUID NOT NULL REFERENCES claims(claim_id) ON DELETE CASCADE,
    document_id UUID REFERENCES claim_documents(document_id) ON DELETE SET NULL,
    line_no INT NOT NULL,
    description VARCHAR(300) NOT NULL,
    part_category VARCHAR(40), -- metal, plastic, rubber, glass, labour
...
CREATE TABLE IF NOT EXISTS fraud_assessments (
    ...
    band VARCHAR(20) NOT NULL
        CHECK (band IN ('clear','monitor','manual_review','siu_referral')),
...
CREATE INDEX IF NOT EXISTS idx_fraud_claim ON fraud_assessments(claim_id, assessed_at DESC);

CREATE TABLE IF NOT EXISTS claim_decisions (
    decision_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    claim_id UUID NOT NULL REFERENCES claims(claim_id) ON DELETE CASCADE,
    outcome VARCHAR(20) NOT NULL
        CHECK (outcome IN ('approved','partially_approved','rejected','referred')),
    payable_amount NUMERIC(14,2) NOT NULL CHECK (payable_amount >= 0),
    rejection_reason VARCHAR(300),
    decided_by VARCHAR(80) NOT NULL,
    is_automated BOOLEAN NOT NULL DEFAULT FALSE,
    override_of UUID REFERENCES claim_decisions(decision_id),
    override_reason TEXT,
    rationale TEXT NOT NULL,
    decided_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_decisions_claim ON claim_decisions(claim_id, decided_at DESC);

CREATE TABLE IF NOT EXISTS settlement_lines (
    id BIGSERIAL PRIMARY KEY,
    decision_id UUID NOT NULL REFERENCES claim_decisions(decision_id) ON DELETE CASCADE,
    line_id BIGINT REFERENCES claim_line_items(line_id) ON DELETE SET NULL,
    component VARCHAR(30) NOT NULL
        CHECK (component IN ('claimed','inadmissible','depreciation','deductible',
                             'coinsurance','sub_limit_cap','sum_insured_cap','payable')),
    amount NUMERIC(14,2) NOT NULL,
    note VARCHAR(200)
);

CREATE INDEX IF NOT EXISTS idx_settlement_decision ON settlement_lines(decision_id);
